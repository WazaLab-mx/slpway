...
function styleContent(html: string): string {
  return html
    ...
    .replace(/<(h[234]|p|ul|li|a|img|hr|blockquote)\b([^>]*)>/gi, (_, tag: string, attrs: string) => {
      ...
      return `<${tag}${cleanAttrs} style="${existing ? `${existing[1] || existing[2]};` : ''}${ELEMENT_STYLES[tag.toLowerCase()]}">`;
    })
...
<tr><td bgcolor="${BLUE}" style="background:${BLUE};padding:30px 24px;border-top:6px solid ${GOLD}">
...
<tr><td style="padding:24px;font-family:${SANS};font-size:16px;line-height:1.65;color:#404040;overflow-wrap:break-word">${content}</td></tr>
<tr><td style="padding:18px 24px;border-top:4px solid ${GOLD};font-family:${SANS};font-size:11px;line-height:1.5;letter-spacing:2px;text-transform:uppercase;color:${BLUE}">Explore. Live. Thrive.</td></tr>
</table></td></tr></table>`;
}
