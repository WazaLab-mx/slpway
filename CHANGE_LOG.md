...
Hero generada con OpenAI gpt-image-1 (mezcalería en bóveda de ladrillo, bartender sirviendo en copita de barro, naranja + sal de gusano), optimizada con sharp (jpg 1280x853 q82, 107KB) → `blog-images/posts/san-luis-potosi-nightlife-hero.jpg`. Contenido base en inglés + traducciones es/de/ja generadas con gpt-5.4 (preservando HTML/clases/links y valores del JSON-LD; nombres de venues/calles sin traducir). Script one-off idempotente: `scripts/insert-nightlife-post.mjs` (lee el HTML del scratchpad vía NL_HTML_PATH, traduce e inserta/actualiza por slug). Verificado: las 4 locales con content+meta completos. Nota: no se pudo previsualizar en dev (server en 500 por recompilación de otro cambio en curso), pero la fila es consistente con posts existentes (misma ruta /blog/[slug]).
